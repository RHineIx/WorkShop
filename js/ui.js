                ...
                <div class="card-footer">
                    <div class="card-price">${price.toLocaleString()} ${symbol}</div>
                     <div class="card-actions">
                        <button class="icon-btn sell-btn" title="بيع"><iconify-icon icon="material-symbols:shopping-cart"></iconify-icon></button>
...
export const populateSyncModal = () => {
    ...
    if (appState.exchangeRate) {
        document.getElementById('exchange-rate').value = appState.exchangeRate;
    }
            ...
            const partNumbersHTML = Object.entries(remote.partNumbers).map(([vendor, code]) => {
                ...
                return `<div class="part-number-row">
                    ...
                    <div class="part-actions">
                        <button class="icon-btn copy-btn" title="نسخ" data-code="${sanitizeHTML(code)}"><iconify-icon icon="material-symbols:content-copy"></iconify-icon></button>
            ...
            return `
                <div class="remote-section">
                    ...
                    <div class="info-badges">
                        ${remote.frequency ? `<div class="info-badge"><iconify-icon icon="material-symbols:wifi-tethering"></iconify-icon><span class="value">${sanitizeHTML(remote.frequency)}</span></div>` : ''}
                        ${remote.fccId ? `<div class="info-badge"><iconify-icon icon="material-symbols:badge-outline"></iconify-icon><span class="value">${sanitizeHTML(remote.fccId)}</span></div>` : ''}
                        ${remote.battery ? `<div class="info-badge"><iconify-icon icon="material-symbols:battery-horiz-075"></iconify-icon><span class="value">${sanitizeHTML(remote.battery)}</span></div>` : ''}
                    </div>
                    <div class="part-numbers-list">${partNumbersHTML}</div>
                    ${remote.notes ? `
                        <div class="notes-section">
                            <div class="notes-header">
                                <iconify-icon icon="material-symbols:sticky-note-2-outline"></iconify-icon>
                                <span>ملاحظات</span>
                            </div>
            ...
            <div class="remote-card" data-id="${car.id}">
                <header class="card-header">
                    ...
                    <div class="header-actions">
                        ...
                        <button class="icon-btn danger delete-btn" title="حذف"><iconify-icon icon="material-symbols:delete"></iconify-icon></button>
                    </div>
                </header>
